...
const DispatcherLoadsTrips: React.FC = () => {
  ...
  return (
    <div className="bg-white p-4 rounded shadow w-full max-w-2xl mb-6">
      <h3 className="font-bold mb-2">All Loads & Trips</h3>
      ...
      <ul>
        {loads.map(load => (
          <li key={load.id} className="mb-2 p-2 border rounded">
            ...
            {load.assignedTo && <div><span className="font-semibold">Assigned To:</span> {load.assignedTo}</div>}
          </li>
        ))}
      </ul>
    </div>
...
export default DispatcherLoadsTrips;
