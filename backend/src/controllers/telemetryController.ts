...
const getFuelStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (!log) return res.status(404).json({ error: 'No telemetry logs found for vehicle' });
    res.json({
    ...
    res.status(500).json({ error: 'Failed to get fuel status', details: (err as any).message });
  }
};

const getNearestStation = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to get nearest station', details: (err as any).message });
  }
};

export { getFuelStatus, getNearestStation };
