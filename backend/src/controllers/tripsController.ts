...
const planRoute = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to plan route', details: (err as any).message });
  }
};
...
const getRouteStatus = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to get route status', details: (err as any).message });
  }
};

const rerouteTrip = async (req: Request, res: Response) => {
  try {
    const { trip_id } = req.params;
    ...
    res.status(500).json({ error: 'Failed to reroute trip', details: (err as any).message });
  }
};

export { planRoute, getRouteStatus, rerouteTrip };
